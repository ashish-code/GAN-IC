...
# select a subset of the celebA dataset
fraction = 10

# ---------------------------------------------------------------------------------------
# Change the path to the location of the celebA data in the local machine
celebA_path = "/home/ash/Data/celebA"
# ---------------------------------------------------------------------------------------
...
for type in ["train","test"]:
	...
	for key in part_dict:
		if part_dict[key]==("0" if type=="train" else "2" if type=="test" else None):
			L.append(key)
	# ----------------------------------------------------------------------------------------
	# sub-sample the images based on fraction
	n_sample = int(len(L)/fraction)
	L = L[:n_sample]
	# ----------------------------------------------------------------------------------------
	count = len(L)
